...
# Customized for Microsoft Endpoint Manager (Intune) with LaunchDaemon self-cleanup.
#
# Deployment model:
#   1. An Intune shell script writes this file to /usr/local/bin/installomator-1st-depnotify.sh
#   2. The same Intune script drops a LaunchDaemon at
#      /Library/LaunchDaemons/com.yourorg.installomator1st.plist that runs this file.
#   3. On successful completion this script:
#        - touches /var/db/.Installomator1stDone  (so it never re-runs)
#        - boots-out and removes the LaunchDaemon (so launchd forgets it)
#        - optionally removes itself from /usr/local/bin
#
# Change the DAEMON_LABEL constant below if you used a different reverse-DNS name
# in your bootstrap script.

# MARK: Variables
instance="intune-1st"   # Name of used instance — shows up in logs and DEPNotify status
LOGO="microsoft"        # "appstore", "jamf", "mosyleb", "mosylem", "addigy", "microsoft", "ws1", "kandji", "filewave"

# MARK: Self-cleanup constants  (edit these to match your bootstrap script)
SELF_PATH="/usr/local/bin/installomator-1st-depnotify.sh"
DAEMON_LABEL="com.yourorg.installomator1st"
DAEMON_PATH="/Library/LaunchDaemons/${DAEMON_LABEL}.plist"
REMOVE_SELF="yes"   # set to "no" if you want to keep the script on disk for inspection
...
scriptVersion="9.11-intune-custom"
# v.  9.11-intune-custom : Customized for Intune deployment with LaunchDaemon self-cleanup
...
# MARK: Self-cleanup function
# Removes the LaunchDaemon that started this script, then optionally removes
# this script itself. Safe to call multiple times — every step is idempotent.
function cleanupLaunchDaemon() {
    printlog "Self-cleanup: removing LaunchDaemon ${DAEMON_LABEL}"

    if [[ -e "$DAEMON_PATH" ]]; then
        # bootout fails silently if the daemon isn't loaded — that's fine
        launchctl bootout "system/${DAEMON_LABEL}" 2>/dev/null || true
        rm -f "$DAEMON_PATH"
        printlog "Self-cleanup: removed $DAEMON_PATH"
    else
        printlog "Self-cleanup: $DAEMON_PATH not present, skipping"
    fi

    if [[ "$REMOVE_SELF" == "yes" && -e "$SELF_PATH" ]]; then
        printlog "Self-cleanup: removing self at $SELF_PATH"
        # Schedule deletion in the background so the script can finish exiting first
...
# MARK: LaunchDaemon self-cleanup
# Tear down the daemon that started us so it doesn't fire again on every boot.
# Runs whether or not we had errors — the conditionFile already prevents re-runs,
# and we want launchd to forget about us either way.
cleanupLaunchDaemon
